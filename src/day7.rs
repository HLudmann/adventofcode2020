...
fn line_to_rule(line: String) -> (String, HashMap<String, usize>) {
    ...
    for bag in bags {
        let capt = RE.captures(bag).and_then(|cap| {Some(cap)}).unwrap().iter().map(|m| m.unwrap().as_str().to_string()).collect::<Vec<String>>();
...
fn can_contain_n_visited<'a>(rules: &'a HashMap<String, HashMap<String, usize>>, bag: &'a String) -> (bool, HashSet<&'a String>) {
    ...
    for k in content.keys() {
        ...
        if k_ok { visited.extend(k_visited) }
    }
    if visited.is_empty() {
        return (false, visited)
    }
...
fn count_bags_in_bag<'a>(rules: &'a HashMap<String, HashMap<String, usize>>, bag: &'a String) -> usize {
    let content = rules.get(bag).unwrap();
    if content.is_empty() {
        return 0
    }
    content.iter().map(|(k, v)| v * (1 + count_bags_in_bag(rules, k))).sum::<usize>()
}

pub fn puzzle1() -> String {
    let mut rules: HashMap<String, HashMap<String, usize>> = HashMap::new();
    for rule in read_lines("./inputs/day7").unwrap().map(|l| l.unwrap()).map(|l| line_to_rule(l)) {
        rules.insert(rule.0, rule.1);
    }
    // println!("rules keys: {:?}", rules.keys());
    let mut can_contain: HashSet<&String> = HashSet::new();
    for bag in rules.keys() {
        if can_contain.contains(bag) { continue }
        let (ok, visited) = can_contain_n_visited(&rules, bag);
        if ok { can_contain.extend(visited) }
    }
...
pub fn puzzle2() -> String {
    ...
    format!("D7P2: {}", count_bags_in_bag(&rules, &"shiny gold".to_string()))
}
